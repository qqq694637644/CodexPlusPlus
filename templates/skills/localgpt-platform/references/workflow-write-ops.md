...
Dispatches a workflow with query parameter `return_run_details=true`, then prefers the Gitea `RunDetails.workflow_run_id` response field when present. Candidate run lookup uses the repo-scoped `/actions/runs` endpoint and filters locally by workflow/ref/actor/created_after; it does not depend on the workflow-scoped `/actions/workflows/{workflow_id}/runs` endpoint. Optional `inputs` must be `object[string,string]`; booleans, numbers, arrays, nested objects, and non-string keys are rejected before calling Gitea. Candidate run lookup normalizes `refs/heads/<branch>` to `branch=<branch>` and omits `branch` for tag or other full refs. If dispatch succeeds but candidate tracking fails, the operation still returns `ok=true` with a warning instead of reporting that dispatch failed.
