...
1. 调用 `gitea_describe_operations` 确认可用 operation 和参数 schema；先用 `detail=brief`，需要单个 schema 时传 `operation` 和 `detail=full`。
2. CI 失败诊断优先使用组合 operation `ci.prepare_failure_context`，它会定位 run、列出失败 jobs，并把失败 job 日志保存到 `{cwd}/jobs/<job_id>/job.log`。
3. 如需单步控制，可使用 `actions.list_runs`、`actions.get_run`、`actions.list_run_jobs`、`actions.download_job_log` 等原子 operation。
4. 如需 artifact，优先使用 `artifact.sync_for_run`；单个 artifact 可用 `actions.download_artifact`。
5. PR 前置检查使用 `pr.preflight`，它只读取 PR metadata、changed files 和 head_sha 相关 CI，不 checkout、不 merge、不评论。
...
| `ci.prepare_failure_context` | 需要 | `cwd` | `run_id`、`branch`、`head_sha`、`status`、`attempt`、`include_artifacts`、`max_failed_jobs`、`page`、`limit` |
| `artifact.sync_for_run` | 需要 | `cwd`、`run_id` | `artifact_name_pattern`、`job_id`、`extract`、`page`、`limit` |
| `pr.preflight` | 需要 | `pr_number` | `page`、`limit`、`ci_limit`、`file_limit` |
...
返回后读取 `data.log_paths` 指向的本地文件。该 operation 不会 rerun、评论 PR 或返回完整日志正文。
...
artifact 会固定写入 `{cwd}/jobs/run-<run_id>/artifact/`，manifest 写入该目录下的 `manifest.json`。
