...
READ_ONLY_OPERATIONS: dict[str, str] = {
    "server.version": "读取 Gitea 服务器版本。",
    "auth.whoami": "读取当前 token 对应用户。",
    "repo.get": "读取仓库元数据。",
    "actions.list_workflows": "列出仓库 Actions workflows。",
    "actions.get_workflow": "读取单个 workflow。",
    "actions.list_runs": "列出 workflow runs。",
    "actions.get_run": "读取单个 workflow run。",
    "actions.list_run_jobs": "列出 run 的 jobs。",
    "actions.get_job": "读取单个 job。",
    "actions.get_job_log": "读取单个 job 日志。",
    "actions.list_artifacts": "列出仓库或 run 的 artifacts。",
    "actions.download_artifact": "下载 artifact zip，并可选解压到本地目录。",
    "actions.list_runners": "列出仓库级 runners。",
}
...
def describe_operations() -> dict[str, Any]:
    return {
        ...
        "operations": [
            ...
            for name, description in sorted(READ_ONLY_OPERATIONS.items())
        ],
        "repo_format": "owner/repo",
        "pagination": "支持 page 和 limit 参数时透传给 Gitea API。",
        "artifact_default_dir": ".gpt-artifacts/runs/<run_id>/artifact-<artifact_id>/",
    }
...
async def download_artifact(client: GiteaClient, repo: str | None, params: dict[str, Any]) -> dict[str, Any]:
    ...
    target_root = safe_target_root(params.get("target_dir"))
...
def safe_target_root(value: Any) -> Path:
    root = Path(str(value or ".gpt-artifacts")).resolve()
    root.mkdir(parents=True, exist_ok=True)
    if not root.is_dir():
        raise PlatformError("invalid_target_dir", "target_dir 不是目录", {"target_dir": str(root)})
    return root
