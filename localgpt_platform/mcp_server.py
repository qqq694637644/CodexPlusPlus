...
INSTRUCTIONS = """
LocalGPT Gitea MCP 只调用 Gitea 官方 /api/v1 REST API，用于 Codex 查询 CI/CD、PR、workflow、runner、cache 边界和 artifact。不要用它执行本地 git、测试、构建或 shell。所有工具返回结构化 JSON：ok 为 true 时读取 data/evidence；ok 为 false 时先处理 error，不要编造结果。不要输出 token、secret 或 runner registration token。

优先使用 gitea_describe_operations 查看 operation 白名单和参数 schema，再调用 gitea_execute。远端写 operation 必须显式传 confirm=true 和 expected_* 参数；读组合 operation 不会暗中写远端。job log 和 artifact 必须传 params.cwd，并固定写入 cwd/jobs/<job_id>/ 下；不要把原始日志或 artifact 内容直接返回给模型。
""".strip()
