...
- `workflow.dispatch_and_track`：触发 workflow dispatch；`workflow_id` 必须是当前 Gitea 支持的 workflow 文件名，例如 `ci.yml`；必须使用 `return_run_details=true` 响应中的 `workflow_run_id` 作为确定匹配，不再做候选 run fallback；`inputs` 必须是 `object[string,string]`，非 string value 会在 MCP schema 层失败；缺少 `workflow_run_id` 视为当前 Gitea API 响应不符合预期并返回错误。
